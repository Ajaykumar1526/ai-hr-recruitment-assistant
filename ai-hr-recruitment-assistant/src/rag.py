...
--------------------------------------------
Builds a small vector knowledge base from markdown files in
data/knowledge_base/ (interview guidelines, skill definitions, recruitment
best practices, role requirements) and retrieves the most relevant chunks
for a given query using ChromaDB + Sentence Transformers embeddings.
"""
...
CHUNK_SIZE = 800  # characters per chunk, simple fixed-size chunking
CHUNK_OVERLAP = 100
...
def build_knowledge_base(force_rebuild: bool = False) -> int:
    """
    Ingest all .md/.txt files from data/knowledge_base/ into the vector store.
    Returns the number of chunks indexed. Safe to call repeatedly (idempotent
    unless force_rebuild=True, in which case the collection is cleared first).
    """
